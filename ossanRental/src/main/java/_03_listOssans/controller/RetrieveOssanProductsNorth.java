...
@WebServlet("/_03_listOssans/DisplayOssanProductsNorth")
// 本控制器負責進行必要的前置作業，以便Dao取回某一頁的商品資料
public class RetrieveOssanProductsNorth extends HttpServlet {
...
//		
		// 讀取瀏覽送來的 pageNo
		String pageNoStr = request.getParameter("pageNoNorth");
		...
		if (pageNoStr == null) {  
			...
			if (cookies != null) {
				...
				for (Cookie c : cookies) {
					if (c.getName().equals(memberId + "pageNoNorth")) {
						try {
							pageNoNorth = Integer.parseInt(c.getValue().trim());
			...
			try {
				pageNoNorth = Integer.parseInt(pageNoStr.trim());
			} catch (NumberFormatException e) {
				pageNoNorth = 1;
			}
		}
		...
		service.setPageNoNorth(pageNoNorth);
		List<String> quoteList = service.getQuoteOssanNorth();
	
		request.setAttribute("ossan_quote", quoteList);
		
		request.setAttribute("baBean", service);
		// service.getPageBooks()方法開始讀取一頁的書籍資料
		Collection<MemberOssanBean> coll = service.getPageOssansNorth();
		session.setAttribute("pageNoNorth", String.valueOf(pageNoNorth));
		request.setAttribute("totalPagesNorth", service.getTotalPagesNorth());
...
//		 -----------------------
		Cookie pnCookie = new Cookie(memberId + "pageNoNorth", String.valueOf(pageNoNorth));
	    // 設定Cookie的存活期為30天
...
//		 『前一頁』、『下一頁』、『最末頁』等資料
		RequestDispatcher rd = request.getRequestDispatcher("listOssansNorth.jsp");
