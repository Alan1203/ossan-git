...
import _02_login.service.impl.LoginServiceImpl;

@WebServlet("/_02_login/new_login.do")
public class LoginServletWithAdmin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("LoginServet Line40");
		// 2. 進行必要的資料轉換
		// 無
		// 3. 檢查使用者輸入資料
		...
		MemberOssanBean mbA = null;
		try {
			// 呼叫 loginService物件的 checkIDPassword()，傳入userid與password兩個參數
			mbA = loginService.checkAdmin(userId, password);
			mb = loginService.checkIDPassword(userId, password);
			
			
			if (mbA != null) {
				// OK, 登入成功, 將mb物件放入Session範圍內，識別字串為"LoginOK"
				session.setAttribute("AdminLoginOK", mbA);
				session.setAttribute("AdminLoginOKString", "管理員成功登入");
			}			
			else {
				
				if (mbA == null && mb != null) {
					int key = loginService.queryOneOssanPKey(userId);
					session.setAttribute("LoginOK", mb);
					session.setAttribute("OssanLoginOK", "大叔會員成功登入");
					session.setAttribute("OssanSessionPKey", key);
